    ...
    with connection:
        ...
        if data:
            ...
            response_body = (
                    ...
                    f'<p>Request Method: {request_method_and_status[0]}</p>\r\n'
                    f'<p>Request Source: {connection.getpeername()}</p>\r\n'
                    f'<p>Response Status: {response_status}</p>\r\n'
                    '<hr>\r\n'
                    '<h4>Request headers:</h4>\r\n' + '\r\n'.join([f"<p>{item}</p>" for item in request_data])
            )
            connection.send(
